        ...
        for batch_size in batch_size_list:
            for epoch in epoch_values:
                ...
                for train_val_index, pseudo_test_index in rkf.split(X_all, y_encoded):
                    ...
                    # 🔢 Tính AUC
                    if num_classes == 2 and set(y_true_labels) >= {0, 1}:
                        auc_score = roc_auc_score(y_true_labels, y_pred_probs[:, 1])
                    elif num_classes > 2 and len(np.unique(y_true_labels)) > 1:
                        auc_score = roc_auc_score(y_true_labels, y_pred_probs, multi_class='ovr', average='macro')
                    else:
                        auc_score = 0.0
...
def main():
    ...
    feature_dir = os.path.join(home_dir, 'data2_SOTA_and_handcrafts_and_BlookNet_optimal_entropy_features')
    result_dir = os.path.join(home_dir, 'training_data2_SOTA_and_handcrafts_and_BlookNet_optimal_entropy_features_v3_v10_2')
    ...
    handcrafted_folders = ['hsv_histograms', 'color_histograms', 'lbp']
    layers = ['block5_conv1', 'block1_conv1', 'block2_conv1', 'block1_conv2']
